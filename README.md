...
| [`prune-comments`](.claude/skills/prune-comments/) | Finds every comment block of four lines or more across first-party source from the cwd down (dependency trees, build output, minified and generated files excluded), then shortens each to three lines or fewer — or deletes it where it only restates the code. Strips notices and guidance aimed at coding agents, and session residue. Two modes: reports for approval in batches by default, or **sweeps** straight through on request and leaves the diff as the review. Comments-only diffs either way, and licence headers, tool directives and type-carrying docblock tags are never touched. Ships a heuristic multi-language scanner (`scripts/find_comments.py`). |
...
| [`show-me`](.claude/skills/show-me/) | **User-invoked only** (`disable-model-invocation`, plus a Step 0 subagent refusal — the carry-back needs a live channel to the user). Explains the most recently posed question, position, or statement as a self-contained visual artifact — one-sentence answer, hand-authored SVG diagrams chosen for what's actually confusing (no Mermaid, matching `code-to-uml`), trade-offs presented fairly — then carries every fork the artifact names back into the session as an `AskUserQuestion` so understanding converts into a decision. Grounds anything repo-related in real code; refuses to manufacture a decision where the artifact presents none. |
| [`research`](.claude/skills/research/) | **User-invoked only** (`disable-model-invocation` + Step 0 subagent refusal — narrow: subagents may gather *within* an invoked session, but never own one). Runs research sessions into a **research notebook** — a directory of dated `YYYY-MM-DD-subject-slug` sessions plus a generated `INDEX.md`. Four modes: new session, continue an existing one, enquire read-only across the corpus, and list. Resolves the notebook root before writing anything (explicit path → `$RESEARCH_NOTEBOOK` → the cwd if it looks like a notebook → a `research/`/`docs/research/` folder in the project → ask) and never scaffolds dated directories into a codebase by guess. Ships session templates and a read-only `scripts/index-rows.sh` that regenerates the index. |
| [`explain-skill`](.claude/skills/explain-skill/) | **User-invoked only** (`disable-model-invocation` + Step 0 subagent refusal). Explains one or more named skills or plugins as a single one-page artifact — badges for who may invoke it, a real options table, an inline-SVG behaviour flow with the gates marked, and a worked input/output example (an artifact-producing skill gets its output embedded as a captioned facsimile). Resolves each target from disk and reads the file rather than recalling it; invents no flags, and labels every illustrative example. Nav bar in the header only when two or more targets are given. |
